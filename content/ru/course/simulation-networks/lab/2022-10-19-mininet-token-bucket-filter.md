...
lastmod: 2022-11-18T16:25:00+03:00
draft: false
weight: 205
toc: true
type: "book"
...
-   Визуализируйте вывод iPerf3 после изменения параметров сети.
...
-   Когда приходит новый пакет, буфер (ведро) проверяется на количество доступных токенов; если доступно хотя бы \\(n\\) токенов, \\(n\\) токенов удаляются из ведра, и пакет отправляется в сеть.
...
-   Алгоритм проиллюстрирован на диаграмме.
...
-   Расчёт всплеска.
    ...
    -   Она должна быть не ниже указанной частоты, делённой на HZ, где HZ --- тактовая частота, настроенная как параметр ядра, и может быть извлечена с помощью следующей команды:
        ...
        ```
    -   HZ на `h1` --- 250. Таким образом, для расчета всплеска делим 10 Гбит/с на 250:
